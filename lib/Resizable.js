...
export default class Resizable extends React.Component<Props, State> {
    ...
    increaseHeightMoveUp = (currentTop, clientY, y, dimension, deltaY) => {
        if (y < this.state.startY) {
            let heightToAdd = Math.abs(deltaY)  //(currentTop - clientY) + this.state.extra
    ...
    reduceHeightMoveDown = (currentTop, clientY, y, dimension, deltaY) => {
        if (y > this.state.startY) {
            //let heightToRemove = (clientY - currentTop) - this.state.extra
            let heightToRemove = deltaY //- this.state.extra
    ...
    increaseWidthMoveLeft = (currentLeft, clientX, x, dimension, deltaX) => {
        if (x < this.state.startX) {
            let widthToAdd = Math.abs(deltaX) //(currentLeft - clientX) + this.state.extra
    ...
    reduceWidthMoveRight = (currentLeft, clientX, x, dimension, deltaX) => {
        if (x > this.state.startX) {
            let widthToRemove = deltaX //(clientX - currentLeft) - this.state.extra
    ...
    isMouseMovingUp = (clientY, currentTop, deltaY) => (
        deltaY < 0) //clientY <= currentTop + this.state.extra)

    isMouseMovingLeft = (clientX, currentLeft, deltaX) => (
        deltaX < 0)//clientX <= currentLeft + this.state.extra)
    ...
    resizeNW = (node, x, y, clientX, clientY, deltaX, deltaY) => {
        const [currentLeft, currentTop] = this.getComponentPosition(node)
        let dimension = this.getCurrentDimension(node)

        dimension = (this.isMouseMovingUp(clientY, currentTop, deltaY) ) ?
            this.increaseHeightMoveUp(currentTop, clientY, y, dimension, deltaY) :
            this.reduceHeightMoveDown(currentTop, clientY, y, dimension, deltaY)

        return this.isMouseMovingLeft(clientX, currentLeft, deltaX) ?
            this.increaseWidthMoveLeft(currentLeft, clientX, x, dimension, deltaX) :
            this.reduceWidthMoveRight(currentLeft, clientX, x, dimension, deltaX)
    }

    resizeNE = (node, x, y, clientX, clientY, canDragX, deltaX, deltaY) => {
        const currentTop = this.getComponentTopPosition(node)
        let dimension = this.getCurrentDimension(node)
        dimension.width = this.state.width + (canDragX ? deltaX : 0);

        return (this.isMouseMovingUp(clientY, currentTop, deltaY) ) ?
            this.increaseHeightMoveUp(currentTop, clientY, y, dimension, deltaY) :
            this.reduceHeightMoveDown(currentTop, clientY, y, dimension, deltaY)
    }

    resizeSW = (node, x, y, clientX, clientY, canDragY, deltaY, deltaX) => {
        let dimension = this.getCurrentDimension(node)
    ...
    getDimensionAcordingToMousePosition = (handleName, node, x, y, clientX, clientY, canDragX, canDragY, deltaX, deltaY) => {
        switch (handleName) {
            case 'react-nw-resizable-handle':
                return this.resizeNW(node, x, y, clientX, clientY, deltaX, deltaY)
            case 'react-ne-resizable-handle':
                return this.resizeNE(node, x, y, clientX, clientY, canDragX, deltaX, deltaY)
            case 'react-sw-resizable-handle':
                return this.resizeSW(node, x, y, clientX, clientY, canDragY, deltaY, deltaX)
